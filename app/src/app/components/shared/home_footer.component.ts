...
export class home_footerComponent {
  ...
  electricity(bh) {
    try {
      ...
      bh = this.scanner(bh);
      //appendnew_next_electricity
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_ioiw4kJ5HujF2xMb');
    }
  }
  ...
  scanner(bh) {
    try {
      ...
      return this.errorHandler(bh, e, 'sd_4F8id4fEsry6Y35k');
    }
  }

  airtime(bh) {
    try {
      let outputVariables = this.getAirtime();

      bh = this.benefciary(bh);
      //appendnew_next_airtime
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_i64vTOHL0OmAQeQr');
    }
  }

  benefciary(bh) {
    try {
      let outputVariables = this.getBeneficiary();

      bh = this.sd_Ii9IutJMHmQi3Mkt(bh);
      //appendnew_next_benefciary
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_2vp8OfdRhLWC4SOu');
    }
  }

  sd_Ii9IutJMHmQi3Mkt(bh) {
    try {
      const page = this.page; // page.contacanated = bh.payed_ben.concat(bh.scanner)
      ...
      //appendnew_next_sd_Ii9IutJMHmQi3Mkt
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_Ii9IutJMHmQi3Mkt');
    }
  }
